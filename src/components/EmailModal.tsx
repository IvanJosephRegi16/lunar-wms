...
export default function EmailModal({ po, items, onClose }: Props) {
  ...
  const openGmail = () => {
    try {
      ...
      // Construct a text representation of the PO, excluding amounts
      const textBody = `PURCHASE ORDER: ${po?.po_number || '—'}
Date: ${new Date().toLocaleDateString('en-IN')}
Vendor: ${po?.vendor || '—'}

REQUIRED MATERIALS:
--------------------------------------------------
${items.map((item, i) => `${i + 1}. ${item.material_code || '—'} | ${item.material_name || '—'}
   Size/Thickness: ${item.size_thickness || '—'}
   Required Qty: ${(item.required_qty ?? item.required_quantity ?? 0).toLocaleString()}`
).join('\n\n')}
--------------------------------------------------

Generated by Lunar's Procurement System`;

      const bodyParam = `&body=${encodeURIComponent(textBody)}`;
      const gmailUrl = `https://mail.google.com/mail/?view=cm&fs=1${toParam}&su=${subject}${bodyParam}`;
