...
export default function EmailModal({ po, items, onClose }: Props) {
  ...
  const openGmail = async () => {
    try {
      ...
      // Temporarily store original styles if we needed to adjust for capture
      const originalTransform = billRef.current.style.transform;
      billRef.current.style.transform = 'none';
      ...
      canvas.toBlob(async (blob) => {
        ...
        try {
          ...
          alert('✅ PO Image Copied to Clipboard!\n\nGmail is opening... Just press Ctrl+V (or Cmd+V) to paste the image directly into the email body.');
      ...
      <div style={{
        ...
        <div ref={billRef} style={{ margin: '20px 28px', borderRadius: '12px', overflow: 'hidden', border: '1px solid #e5e7eb' }}>
          ...
          <div style={{ background: '#f8fafc', padding: '20px 36px', display: 'flex', gap: '32px', borderBottom: '1px solid #e5e7eb' }}>
            ...
            <div style={{ flex: 1 }}>
              {/* Removed Net Amount per request */}
            </div>
          </div>
