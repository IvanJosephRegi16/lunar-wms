...
export default function PODashboard() {
  ...
  return (
    <div className={styles.vortexPo}>
      ...
      {/* 7. Supervisor Messages Panel — PM Only */}
      {user?.role === 'pm' && (() => {
        const supervisorMsgs = notifications.filter(n => n.type === 'partial_entry');
        if (supervisorMsgs.length === 0) return null;
        return (
          <div style={{ marginTop: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
              <h3 className={styles.chartTitle} style={{ margin: 0 }}>📩 Messages from Supervisor</h3>
              <span style={{ background: '#fef3c7', color: '#92400e', fontSize: '11px', fontWeight: 900, padding: '3px 10px', borderRadius: '20px', border: '1px solid #fde68a' }}>
                {supervisorMsgs.filter(n => !n.is_read).length} Unread
              </span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {supervisorMsgs.map((n: any) => (
                <div key={n.id} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', flexWrap: 'wrap' }}>
                      <div>
                        <span style={{ fontSize: '13px', fontWeight: 900, color: '#1e40af', fontFamily: 'monospace' }}>
                          {n.po_number}
                        </span>
                        <span style={{ fontSize: '11px', fontWeight: 700, color: '#f59e0b', background: '#fffbeb', border: '1px solid #fde68a', padding: '2px 8px', borderRadius: '12px', marginLeft: '8px' }}>
                          Partial Entry
                        </span>
                        {!n.is_read && (
                          <span style={{ fontSize: '10px', fontWeight: 900, color: '#dc2626', background: '#fee2e2', border: '1px solid #fca5a5', padding: '2px 8px', borderRadius: '12px', marginLeft: '6px' }}>
                            NEW
                          </span>
                        )}
                      </div>
                      <span style={{ fontSize: '11px', color: '#94a3b8', fontFamily: 'monospace', flexShrink: 0 }}>
                        {new Date(n.created_at).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}
                      </span>
                    </div>
                    <p style={{ margin: '8px 0 0', fontSize: '13px', color: '#374151', lineHeight: '1.6', fontWeight: 500 }}>
                      {n.message}
                    </p>
                    <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
                      <Link
                        ...
                        style={{
                          fontSize: '12px', fontWeight: 800, color: '#1d4ed8',
                          background: '#eff6ff', padding: '6px 14px', borderRadius: '8px',
                          textDecoration: 'none', border: '1px solid #bfdbfe', transition: 'all 0.2s'
                        }}
                      >
                        View PO Details →
                      </Link>
                      {!n.is_read && (
                        <button
                          ...
                          style={{
                            ...
                            background: '#f3f4f6', padding: '6px 14px', borderRadius: '8px',
